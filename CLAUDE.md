...
```
backend/shared/scraper/
  base.py              # AbstractScraper
  types.py             # StructuredQuery, RawListing
  manager.py           # get_scraper() factory
  plugins/             # drop *.py here — auto-discovered
...
Plugin is auto-discovered — drop `mysource.py` in `backend/shared/scraper/plugins/`.
